...
CDuiFrameWnd::CDuiFrameWnd() :flag(1){}
LPCTSTR    CDuiFrameWnd::GetWindowClassName() const   { return _T("DUIMainFrame"); }
CDuiString CDuiFrameWnd::GetSkinFile()                { return _T("WinDui.xml"); }
CDuiString CDuiFrameWnd::GetSkinFolder()              { return _T(""); }
void CDuiFrameWnd::Init()
{//初始化工作，控件对接,确保控件指针有所指的实体
	...
	percent = static_cast<CLabelUI*>(m_PaintManager.FindControl(_T("percent")));
...
COLORREF CDuiFrameWnd::ColorConvert(COLORREF &clr, DWORD Mode)//(ABGR)-->(ARGB);A-->FF
{//颜色转换
	//Mode=0:透明
	//Mode=1:不透明
	//小端模式
...
void CDuiFrameWnd::doDraw(HDC hdc, POINTS centre, DWORD radian, float start, float angle)//角度，所占比例*360=度数
{//画圆弧线图
...
void CDuiFrameWnd::OnPaint(HWND hWnd)
{//完整的画饼图事件
	...
	for (UINT i = 0; i < 5; i++)
	{
		hPen[i] = CreatePen(PS_SOLID, 4, ColorConvert(color[i], 0));
		hOldPen[i] = (HPEN)SelectObject(hdc, hPen[i]);

		for (UINT j = 0; j < 12; j++)
		{//重复画弧线动作
			if (percentAngle[i])
				doDraw(hdc, centre, r += 3, startAngle, percentAngle[i]);
		}
...
void CDuiFrameWnd::GetButtonColor(UINT i, CDuiString  strName)
{//用户更改按钮颜色
...
void CDuiFrameWnd::ColorForDraw()
{//获取颜色用来画饼图
	for (UINT i = 0; i < 5; i++)
	{
...
ULONGLONG CDuiFrameWnd::TraverseDir(CString& dir)
{//遍历文件夹函数,返回字节数,可能偏小
...
ULONGLONG CDuiFrameWnd::GetFolderSize()
{//按下设置路径按钮后的一系列操作
	BROWSEINFO lpbi;
...
void CDuiFrameWnd::GetUserPercentAngle(UINT i, ULONGLONG size)
{//获取可变路径的角度比例
	//GetDiskCap();
...
void CDuiFrameWnd::GetPercentAngle()
{//获取比例，测试阶	,如果有bug，直接开放这里
	...
	swprintf_s(szText, _T("%0.2f%%"), (percentAngle[4]/360)*100);
	percent->SetText(szText);
}
